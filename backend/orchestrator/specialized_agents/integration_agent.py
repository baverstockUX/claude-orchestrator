"""Integration agent for third-party API integrations and webhooks."""

import logging
import re

from backend.llm.prompt_templates import get_integration_agent_prompt
...
class IntegrationAgent(WorkerAgent):
    """Agent specialized in third-party API integrations."""

    async def _invoke_llm_for_task(self, task: Task) -> str:
        """
        Invoke LLM with integration-specific prompt.

        Args:
            task: Task to execute

        Returns:
            LLM response with integration code
        """
        ...
        # Generate prompt
        prompt = get_integration_agent_prompt(
            task=task,
        ...
        response = self.bedrock.invoke_model(
            prompt=prompt,
            system_prompt=(
                "You are an expert in API integration development, specializing in RESTful APIs, "
                "webhooks, authentication flows (OAuth, API keys), and robust error handling. "
                "You write secure, maintainable integration code with proper retries and logging."
            ),
            max_tokens=8000
        )
    ...
    async def _apply_changes(self, llm_response: str, task: Task) -> list[str]:
        """
        Parse LLM response and apply changes to integration files.
        ...
        Args:
            llm_response: LLM response with integration code
        ...
        code_block_pattern = r'```(?:python|typescript|ts|javascript|js)?\n(.*?)```'
        ...
        if not file_markers:
            ...
            if len(task.files_to_create) == 1:
                ...
                raise FileOperationError(
                    ...
                    "Expected format: # filepath: path/to/file.py"
                )
        else:
            # Process each file
