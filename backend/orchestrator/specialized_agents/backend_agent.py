"""Backend agent for FastAPI + SQLAlchemy tasks."""

import logging
import re

from backend.llm.prompt_templates import get_backend_agent_prompt
...
class BackendAgent(WorkerAgent):
    """Agent specialized in FastAPI + SQLAlchemy development."""

    async def _invoke_llm_for_task(self, task: Task) -> str:
        """
        Invoke LLM with backend-specific prompt.

        Args:
            task: Task to execute

        Returns:
            LLM response with FastAPI code
        """
        ...
        # Generate prompt
        prompt = get_backend_agent_prompt(
            task=task,
        ...
        response = self.bedrock.invoke_model(
            prompt=prompt,
            system_prompt=(
                "You are an expert FastAPI developer specializing in async Python, SQLAlchemy, "
                "and RESTful API design. You write clean, well-typed Python code with proper "
                "error handling, validation, and database best practices."
            ),
            max_tokens=8000
        )
    ...
    async def _apply_changes(self, llm_response: str, task: Task) -> list[str]:
        """
        Parse LLM response and apply changes to Python files.
        ...
        Args:
            llm_response: LLM response with Python code
